...
class SimpleTrainer(TrainerBase):
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            output = self.model_inference(image)  ######### edit for model input 
            # output = self.model_inference(image, caption, label)#ZSL
    ...
    def model_inference(self, input, caption, label):    ######### edit for model input
        return self.model(input, caption, label) #ZSL

    # def model_inference(self, input):    ######### edit for model input
    #     return self.model(input) #ZSL

    def parse_batch_test(self, batch):
        # Extract the impath list from the batch
        impath_list = batch["impath"]

        # Get the original image and label from the batch
        image = batch["img"].to(self.device)  # Original image
        label = batch["label"].to(self.device)  # Label

        # Initialize lists to store the augmented images, captions, and knowledge graph embeddings
        captions = []

        # Iterate over each image path in the impath_list
        for i, impath in enumerate(impath_list):
            # Derive the path for the augmented image and caption
            caption_path = impath.replace("/images/", "/caption_images/").replace(".jpg", ".pt")

            caption = torch.load(caption_path).to(self.device)

            # Get the corresponding label for this batch item
            current_label = os.path.basename(os.path.dirname(impath))

            # Append to the lists
            captions.append(caption)

        # Stack the list of captions into tensors
        captions = torch.stack(captions)

        # Return all components needed for forward pass
