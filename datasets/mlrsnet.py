...
def read_and_split_data(image_dir, p_trn=0.5, p_val=0.2, ignored=None, new_cnames=None):
    if ignored is None:
        ignored = []  # Ensure ignored is a list
    ...
    for label, category in enumerate(categories):
        ...
        images = listdir_nohidden(class_dir)
        random.shuffle(images)  # Shuffle to ensure randomness

        num_total = len(images)
        num_train = int(p_trn * num_total)
        ...
        for img in train_images:
            train.append(Datum(impath=os.path.join(class_dir, img), label=label, classname=category))

        for img in val_images:
            val.append(Datum(impath=os.path.join(class_dir, img), label=label, classname=category))

        for img in test_images:
            test.append(Datum(impath=os.path.join(class_dir, img), label=label, classname=category))
